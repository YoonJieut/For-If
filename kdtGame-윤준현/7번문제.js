...
// 1과 8을 비교해서 오른쪽으로 보냅니다.
// 이긴 것이 3번째 것과 비교해서 오른쪽으로 보내주세요
// 또 이긴 것이 4번째 것과 비교해서 오른쪽으로 보내주세요
// 또 이긴 것을 5번째 것과 비교에서 마지막 배열에 넣어주세요.

// for( let i =0; i < ary.length; i++) {
//   if( ary[i] < ary[i+1] ) {
//     // a[i+1] 를 오른쪽으로 보내주세요(현상 유지)
//   } else if ( i+1 === ary.length ){
//     break;
//   } else {
//     // a[i] 를 오른쪽으로 보내주세요(교환)
//     swiching = ary[i];
//     ary[i] = ary[i+1];
//     ary[i+1] = swiching;
//   }
...
for( let i =0; i < _pokemon.length; i++) {
    if( _pokemon[i] < _pokemon[i+1] ) {
      // a[i+1] 를 오른쪽으로 보내주세요(현상 유지)
    } else if ( i+1 === _pokemon.length ){
      break;
    } else {
      // a[i] 를 오른쪽으로 보내주세요(교환)
      swiching[0] = _pokemon[i];
      _pokemon[i] = _pokemon[i+1];
      _pokemon[i+1] = swiching[0];
    }
  }
